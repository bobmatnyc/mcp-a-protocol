...
*Reference*: SPEC.md §1a "RESOLVED (beta):" block.
...
*Reference*: SPEC.md §2 query request, Structured-Response Mode sketches.
...
*Reference*: SPEC.md §2 query options, Structured-Response Mode conformance.
...
This is not a recount; it's a clarification. The spec has always treated `schema` as a core element (§1a is substantive); MAEP-0001 formalizes its independence and resolves the ambiguity.
...
- **SPEC.md**: §1 (discover), §1a (schema), §2 (query), §3 (follow_up), §4 (context), §5 (explain), §Aggregation Correctness Conformance, §Open Questions.
