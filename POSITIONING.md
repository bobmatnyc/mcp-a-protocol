...
MCP-A is **not** a competitor or a sibling to MCP -- it is a **profile of MCP**. Every MCP-A server is an MCP server; every MCP-A client is an MCP client. MCP-A defines specific tools (the five primitives) and result shapes designed to make LLM processing efficient.
...
The [Agentic AI Foundation](https://www.agenticaifoundation.org/) (hypothetical; represents the broader community) is exploring standards for multi-agent systems, tool protocols, and orchestration.

**MCP-A's position**:

- **Primary venue** (near term): Standalone GitHub spec repo (`a public MCP-A spec repo` or similar). Lowest friction, full authorial control, MAEP (MCP-A Enhancement Proposal) process mirrors MCP's SEP.
...
3. AAIF SEP process may add requirements (e.g., reference implementation) that we're not ready for yet.
...
- **Not specific to Duetto.** It's a public standard, usable by any organization with multi-source data.
