...
The MAEP process is modeled on the Specification Enhancement Proposal (SEP) process used by the [Model Context Protocol](https://modelcontextprotocol.io/) and inspired by Python's PEP process. Because MCP-A is an MCP profile, MAEPs are designed to graduate into MCP's own SEP track at the AAIF/Linux Foundation when upstreamed -- the long-term home is the MCP SEP process, and the standalone `mcp-a-spec` repo is the fast near-term path. It ensures:
...
**Repository**: a public spec repository (e.g., `github.com/agentic-ai/mcp-a-spec`) (or similar org)

**Structure**:
```
mcp-a-spec/
├── spec/
│   ├── 0001-initial-five-primitives.md
│   ├── 0002-webhook-async.md
│   └── ...
├── README.md (links to all MAEPs)
├── GOVERNANCE.md (this process)
...
**Strategy**: Use the standalone repo (a public MCP-A spec repository) for rapid iteration and proven battle-testing. Once stable and proven, propose to AAIF/MCP as a profile and graduate MAEPs into the MCP SEP track. The standalone repo remains the fast near-term path; the MCP SEP process at AAIF/Linux Foundation is the long-term home.
...
Before writing, search the `mcp-a-spec/spec/` directory for related MAEPs. (Avoid proposing what's already in flight.)
...
Open an issue in the `mcp-a-spec` repo titled "MAEP-NNNN: [Title]". Link to your draft MAEP file. Describe:
...
- **Authority**: MCP-A spec maintainer (Bob) has final say if consensus is not reached.
...
**Bob Matsuoka** (Robert Matsuoka, CTO at Duetto Research) is the initial BDFL (Benevolent Dictator For Life) for MCP-A. He:
...
- **Website** (future): Static documentation site (e.g., `mcp-a-spec.io`).
...
**Week 4**: Maintainer (Bob) reviews and comments: "I like the direction. One concern: how do we version DSLs independently from the spec?" Author updates the rationale section to address this.
...
**Q: Can I propose a major new primitive (e.g., MAEP-0010 adds a 6th primitive)?**
...
A: At that point, we'd align with AAIF governance. The current MAEP process is for the standalone era. Because MCP-A is an MCP profile, the long-term move is to graduate MAEPs into MCP's own SEP track at the AAIF/Linux Foundation -- the standalone `mcp-a-spec` repo is the fast near-term path until then.
